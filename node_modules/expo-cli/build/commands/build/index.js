...
function _load_WebBuilder() {
  return _WebBuilder = _interopRequireDefault(require('./WebBuilder'));
}
...
exports.default = function (program) {
  ...
  program.command('build:web [project-dir]').option('--no-polyfill', 'Prevent webpack from including @babel/polyfill').option('-d, --dev', 'Bundle your project using webpack in dev mode.').option('--stats <path>', 'Output path for webpack stats. Defaults to "web-build-stats.json"', 'web-build-stats.json').description('Build a production bundle for your project, compressed and ready for deployment.').asyncActionProjectDir(function (projectDir, options) {
    var channelRe = new RegExp(/^[a-z\d][a-z\d._-]*$/);
    if (!channelRe.test(options.releaseChannel)) {
      (_log || _load_log()).default.error('Release channel name can only contain lowercase letters, numbers and special characters . _ and -');
      process.exit(1);
    }
    var webBuilder = new (_WebBuilder || _load_WebBuilder()).default(projectDir, options);
